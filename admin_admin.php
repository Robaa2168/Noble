...
$shares_available = "SELECT sum(total_balance) as shares_available FROM coins WHERE due_date < CURRENT_DATE() AND control_bal = 0 ";
                      $addition = mysqli_query($conn, $shares_available);
                      $checking = mysqli_num_rows($addition);
                      while ($row = mysqli_fetch_assoc($addition)) {
                      $number2 = $row['shares_available'];
}
...
if (isset($_POST['submit'])) {
  ...
  $query_admin = "SELECT * from customers where username = '$admin_user'";
  $result_admin = mysqli_query($conn, $query_admin);
  if (mysqli_num_rows($result_admin) > 0) {
    if ($row = mysqli_fetch_assoc($result_admin)) {
      $user_id = $row['customer_id'];
      $customer_contact_admin = $row['customer_contact'];
      $customer_username_admin = $row['username'];
      $referral_admin = $row['referral'];
      $customer_email_admin = $row['customer_email'];
      $customer_confirm_code_admin = $row['customer_confirm_code'];
    }
  }
  $query_pending = "SELECT * FROM pairs WHERE customer_id = $user_id  AND payment_status='pending' ";
  ...
  ?>
    ...
    $boughtref = "SELECT sum(earning) as pay FROM referral WHERE 
    order_status=$customer_confirm_code_admin AND initial= 0";
    ...
    if ($totalmon > (0.15 * $Amount)) {
      ...
      $remainder = $totalmon - (0.10 * $totalmon);
      $sql8 = "UPDATE customers
      SET Savings = $remainder WHERE customer_email ='$customer_email_admin'";
      // Prepare statement
      ...
      SET Savings = 0 WHERE customer_email ='$customer_email_admin'";
      // Prepare statement
  ...
  SET initial = 1 WHERE order_status = $customer_confirm_code_admin ";
  // Prepare statement
...
VALUES(:customer_id,:invoice_no,:phone,:order_cost,:cost_balance,:order_date,:due_date,:order_total,:total_balance,:control_bal,:Customer_email)";
  ...
  // Binding Post Values
  $query->bindParam(':customer_id', $user_id, PDO::PARAM_STR);
  $query->bindParam(':invoice_no', $random, PDO::PARAM_STR);
  $query->bindParam(':phone', $customer_contact_admin, PDO::PARAM_STR);
  ...
  $query->bindParam(':Customer_email', $customer_username_admin, PDO::PARAM_STR);
  $query->execute();
  $lastInsertId = $dbh->lastInsertId();
  if ($lastInsertId) {
    $query = "SELECT * FROM customers WHERE customer_email='$customer_email_admin'";
    ...
    if (mysqli_num_rows($result) > 0) {
      if ($row = mysqli_fetch_assoc($result)) {
        ...
        // Binding Post Values
        $queryinsertref->bindParam(':customer_id', $user_id, PDO::PARAM_STR);
        $queryinsertref->bindParam(':earning', $wallet5, PDO::PARAM_STR);
        $queryinsertref->bindParam(':username', $customer_username_admin, PDO::PARAM_STR);
        $queryinsertref->bindParam(':order_status', $referral_admin, PDO::PARAM_STR);
  ...
  AND due_date < CURRENT_DATE() AND customer_id != '$user_id' AND control_bal = 0
  LIMIT 1";
...
VALUES(:invoice_no,:customer_id,:sell_id,:order_no,:initial,:order_cost,:phone,:Customer_email)";
        ...
        // Binding Post Values
        $queryinsert->bindParam(':customer_id', $user_id, PDO::PARAM_STR);
        $queryinsert->bindParam(':sell_id', $lastInsertId, PDO::PARAM_STR);
        $queryinsert->bindParam(':invoice_no', $random, PDO::PARAM_STR);
        $queryinsert->bindParam(':phone', $customer_contact_admin, PDO::PARAM_STR);
        ...
        $queryinsert->bindParam(':Customer_email', $customer_username_admin, PDO::PARAM_STR);
        $queryinsert->execute();
...
<div class="page-wrapper">
  <div class="page-content">
    ...
    <div class="row row-cols-2 row-cols-md-2 row-cols-xl-4">
      ...
      $bought = "SELECT sum(order_cost) as paid FROM pairs ";
      ...
      $sold = "SELECT sum(order_total) as sold FROM coins where total_balance < 1";
    ...
    <div class="container d-block d-lg-none ">
      ...
      <div class="col text-center col-sm-6 col-md-6 col-lg-6 col-xl-6 mt-4 ml-8" style="text-align:center">
        <h3 style="color:#191970;font-size:16px;padding:8px;background:#f0f0f0;border-radius:5px" id="tm1">MPESA AUCTION </h3><br>
        ...
        <?php if ($customer_role3 == 3) { ?>
          <a href="turnoff.php" aria-expanded="false" class="btn btn-sm btn-danger"> OFF</a>
        <?php } else { ?>
          <a href="turnon.php" aria-expanded="false" class="btn btn-sm btn-success"> ON</a>
        <?php } ?>
  ...
  <div class="col text-center">
    <a class="btn btn-sm btn-danger" data-toggle="modal" data-target="#exampleModal" aria-expanded="false">Invest Now</a>
  </div>
</div>
<style>
...
<div class="container">
  ...
  <div class="receipt">
    ...
    <p class="comprobe">invest button will appear here</p>
  </div>
</div>
      ...
      <div class="modal-body ">
        <form action='' method='post'>
          <div class="form-group">
            ...
            <input type="text" name="username" class="form-control" placeholder="username" value="<?php echo @$_POST["username"]; ?>" required>

            <label for="recipient-name" class="col-form-label">Amount:</label>
            <input type="number" name="amount" class="form-control" id="recipient-name" placeholder="Amount" value="<?php echo @$_POST["amount"]; ?>" required>
          </div>
