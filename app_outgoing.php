...
$sql12 =  "SELECT * from coins where  order_id= '$refer'"; 
  ...
  $sql15 =  "SELECT * from pairs where customer_id = $customer_idi AND  order_no = '$refer'"; 
...
<?php
 ...
 if ($rows['payment_status'] == "pending"){ ?>
					   ...
					   <a type="button" href=""
					   class="btn btn-outline-warning btn btn-sm text-warning" name="confirm">In progress </a>
                       </div>
                       <?php } else { ?> 
									...
									<main>
										<div class="row contacts">
											<div class="col invoice-to">
												...
												<div>Invoice:#<?php echo $rows['invoice_no'];?></div>
<div>Username: <?php echo $rows['Customer_email'];?></div>
<div>Phone: <?php echo $rows['phone'];?></div>
<div> <strong> Amount: <?php echo $rows['order_cost'];?></strong></div>
												</div>
											
											<div class="col invoice-details">
												<h1 class="invoice-id">INVOICE</h1>
												<div class="date">Date of Invoice: <?php echo $rows['bid_time'];?></div>
												<div class="date">Due Date: <?php echo $rows['bid_time'];?></div>
											</div>
										</div>
										<hr>
